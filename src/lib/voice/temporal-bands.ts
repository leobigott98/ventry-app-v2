...
function parseDate(text: string, referenceDate: string) {
  if (/\bmañana\b/.test(text)) return addCalendarDays(referenceDate, 1);
  if (/\bhoy\b/.test(text)) return referenceDate;
  for (const [name, weekday] of Object.entries(WEEKDAYS)) if (new RegExp(`\\b${name}\\b`).test(text)) return nextWeekday(referenceDate, weekday);
  return /\b(20\d{2}-\d{2}-\d{2})\b/.exec(text)?.[1] ?? null;
}
...
const explicitPeriod = (text: string) => /(?:de la |en la )(mañana|tarde|noche|madrugada)/.exec(text)?.[1];
...
export function interpretVenezuelanTemporal(input: { transcript: string; referenceLocalDate: string; dateText?: string | null; arrivalText?: string | null; plannedExitText?: string | null }): DeterministicTemporalResult {
  ...
  const arrivalText = normalize(input.arrivalText ?? transcript);
  ...
  if (/\btodo el día\b|\btodo el dia\b/.test(arrivalText)) arrivalWindowMode = "all_day";
  else {
    ...
    if (band) {
      ...
      const between = /(?:entre|de)\s+(?:las\s+)?(\d{1,2})(?:[:.](\d{2}))?\s*(?:de la\s+)?(mañana|tarde|noche|madrugada)?\s+(?:y|a)\s+(?:las\s+)?(\d{1,2})(?:[:.](\d{2}))?\s*(?:de la\s+)?(mañana|tarde|noche|madrugada)?/.exec(arrivalText);
      if (between) {
        ...
        const from = /(?:desde|a|como a|tipo|alrededor de)\s+(?:las\s+)?(\d{1,2})(?:[:.](\d{2}))?\s*(?:de la\s+)?(mañana|tarde|noche|madrugada)?/.exec(arrivalText);
        if (from) {
          const period = from[3] ?? explicitPeriod(arrivalText); arrivalWindowMode = "from_time";
          if (!period && Number(from[1]) >= 1 && Number(from[1]) <= 6) {
            const morning = hour24(from[1], from[2], "mañana")!;
            const afternoon = hour24(from[1], from[2], "tarde")!;
            ambiguities.push({ field: "arrivalStart", code: "AM_PM_AMBIGUOUS", question: `¿Las ${Number(from[1])} son de la mañana o de la tarde?`, options: [{ value: morning, label: `${morning} de la mañana` }, { value: afternoon, label: `${afternoon} de la tarde` }] });
          } else {
        ...
        const before = /antes de\s+(?:las\s+)?(\d{1,2})(?:[:.](\d{2}))?\s*(?:de la\s+)?(mañana|tarde|noche|madrugada)?/.exec(arrivalText);
        if (before) {
  ...
  const exit = /(?:salir|salida|salgan|debería salir|deberian salir|deberían salir)[^\d]{0,20}(?:como a|tipo|alrededor de|a)?\s*(?:las\s+)?(\d{1,2})(?:[:.](\d{2}))?\s*(?:de la\s+)?(mañana|tarde|noche|madrugada)?/.exec(plannedExitText);
  if (exit && visitDate) {
    const period = exit[3] ?? explicitPeriod(plannedExitText); plannedExitTime = hour24(exit[1], exit[2], period);
    if (!period && arrivalStart && plannedExitTime && plannedExitTime <= arrivalStart && Number(exit[1]) <= 6) plannedExitTime = hour24(exit[1], exit[2], "tarde");
