...
export default async function InvitationGroupPage({ params }: { params: Promise<{ groupId: string }> }) {
  ...
  const activeCount = group.invitations.filter((item) => ["active","scheduled"].includes(getInvitationEffectiveStatus(item))).length;
