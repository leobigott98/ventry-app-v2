...
export default async function InvitationsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (resident) return <section className="min-h-[100dvh]"><ResidentPageHeader subtitle={`${result.total} ${result.total === 1 ? "invitación" : "invitaciones"}`} title="Mis invitaciones" /><div className="space-y-4 px-4 py-4 sm:px-6">{content}</div></section>;
