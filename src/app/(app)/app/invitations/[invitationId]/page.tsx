...
export default async function InvitationDetailPage({ params, searchParams }: { params: Promise<{ invitationId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const body = <div className="space-y-4">
    {query.contactWarning === "1" ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900" role="status">La invitación se creó correctamente, pero no pudimos guardar el contacto. Puedes intentarlo luego desde Contactos.</div> : null}
