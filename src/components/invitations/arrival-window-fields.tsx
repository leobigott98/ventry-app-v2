...
export function ArrivalWindowFields({
  ...
  return <div className="space-y-6">
    <div className="space-y-2">
      ...
      <Input className="h-14 text-base" id={`${idPrefix}-date`} min={minDate} type="date" value={value.date} onChange={(event) => onChange("date", event.target.value)} />
      {error("date")}
    </div>
    ...
    {value.arrivalWindowMode === "from_time" ? <div className="space-y-4 rounded-2xl bg-surface p-4">
      ...
      {!hasArrivalEnd ? <Button className="min-h-11" onClick={() => { onChange("arrivalEndDate", value.date); onChange("arrivalEnd", null); }} type="button" variant="ghost"><Plus className="h-4 w-4" /> Agregar hora límite</Button> : <fieldset className="space-y-3"><legend className="font-semibold">Hora límite de llegada</legend><div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label htmlFor={`${idPrefix}-end-date`}>Fecha</Label><Input className="h-14" id={`${idPrefix}-end-date`} min={value.date} type="date" value={value.arrivalEndDate ?? ""} onChange={(event) => onChange("arrivalEndDate", event.target.value || null)} />{error("arrivalEndDate")}</div><div className="space-y-2"><Label htmlFor={`${idPrefix}-end`}>Hora</Label><Input className="h-14" id={`${idPrefix}-end`} type="time" value={value.arrivalEnd ?? ""} onChange={(event) => onChange("arrivalEnd", event.target.value || null)} />{error("arrivalEnd")}</div></div><Button className="min-h-11" onClick={() => { onChange("arrivalEndDate", null); onChange("arrivalEnd", null); }} type="button" variant="ghost"><X className="h-4 w-4" /> Quitar hora límite</Button></fieldset>}
    </div> : null}

    {!hasPlannedExit ? <Button className="min-h-12 w-full justify-start rounded-2xl border-dashed" onClick={() => { onChange("plannedExitDate", value.date); onChange("plannedExitTime", null); }} type="button" variant="outline"><Clock3 className="h-5 w-5" /> Agregar salida prevista</Button> : <fieldset className="rounded-2xl border border-border bg-surface p-4"><legend className="px-1 font-bold">Salida prevista</legend><p className="mb-3 text-sm text-muted-foreground">Es informativa y no modifica la vigencia de llegada.</p><div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label htmlFor={`${idPrefix}-exit-date`}>Fecha</Label><Input className="h-14" id={`${idPrefix}-exit-date`} min={value.date} type="date" value={value.plannedExitDate ?? ""} onChange={(event) => onChange("plannedExitDate", event.target.value || null)} />{error("plannedExitDate")}</div><div className="space-y-2"><Label htmlFor={`${idPrefix}-exit-time`}>Hora</Label><Input className="h-14" id={`${idPrefix}-exit-time`} type="time" value={value.plannedExitTime ?? ""} onChange={(event) => onChange("plannedExitTime", event.target.value || null)} />{error("plannedExitTime")}</div></div><Button className="mt-3 min-h-11" onClick={() => { onChange("plannedExitDate", null); onChange("plannedExitTime", null); }} type="button" variant="ghost"><X className="h-4 w-4" /> Quitar salida prevista</Button></fieldset>}
  </div>;
}
