...
function CompanionLimitSelect({ ariaLabel, id, onChange, value }: { ariaLabel?: string; id?: string; onChange: (value: number) => void; value: number }) {
  return <Select aria-label={ariaLabel} className="h-12 w-full min-w-0 max-w-full" id={id} value={String(value)} onChange={(event) => onChange(Number(event.target.value))}>
    {[1, 2, 3, 4, 5].map((amount) => <option key={amount} value={amount}>{amount} {amount === 1 ? "acompañante" : "acompañantes"}</option>)}
  </Select>;
}
...
export function EventForm({ contactAutocomplete = false, residents, timeZone = APP_TIME_ZONE }: { contactAutocomplete?: boolean; residents: ResidentOption[]; timeZone?: string }) {
  ...
  return <form className="min-h-[100dvh] w-full min-w-0 max-w-full bg-background" onSubmit={(event) => event.preventDefault()}>
    <ResidentPageHeader backHref="/app/events" progress={{ current: step, total: 4 }} title="Nuevo evento" />
    <div className="grid w-full min-w-0 max-w-full gap-8 px-5 pb-40 pt-7 sm:px-7 md:px-8 md:pb-8 xl:grid-cols-[minmax(0,42rem)_minmax(18rem,24rem)] xl:px-10">
      <main className="w-full min-w-0 max-w-2xl">
        ...
        {step === 3 ? <div className="mt-6 space-y-4">
          <div className="min-w-0 max-w-full rounded-2xl border border-border bg-surface p-4"><label className="flex min-h-11 items-center gap-3 font-bold"><input className="h-5 w-5 shrink-0 accent-[#1446cc]" checked={form.allowsCompanions} onChange={(event) => { const enabled=event.target.checked; setForm((current) => ({ ...current, allowsCompanions: enabled, maxCompanions: enabled ? Math.max(current.maxCompanions,1) : 0 })); setDraftGuest((current) => ({ ...current, allowsCompanions: enabled, maxCompanions: enabled ? Math.max(current.maxCompanions ?? 0,1) : 0 })); }} type="checkbox" />Permitir acompañantes por defecto</label>{form.allowsCompanions ? <div className="mt-3 min-w-0 max-w-full space-y-2"><Label htmlFor="eventMaxCompanions">Máximo por invitado</Label><CompanionLimitSelect id="eventMaxCompanions" value={form.maxCompanions} onChange={(value) => { setForm((current) => ({ ...current,maxCompanions:value })); setDraftGuest((current) => ({...current,maxCompanions:value})); }} /></div> : null}</div>
          ...
          {guestMode === "manual" ? <div className="min-w-0 max-w-full space-y-3 rounded-2xl bg-surface p-4"><div className="min-w-0 max-w-full space-y-2"><Label htmlFor="guestName">Nombre completo</Label><ContactAutocomplete ariaLabel="Nombre completo" className="h-14 w-full min-w-0 max-w-full text-base" enabled={contactAutocomplete} id="guestName" value={draftGuest.fullName} onChange={(fullName) => setDraftGuest((current) => ({ ...current, fullName }))} onEnter={addGuest} onSelect={(contact: ResidentContactViewModel) => { const candidate = { ...draftGuest, fullName: contact.name, phone: contact.phone ?? draftGuest.phone, residentContactId: contact.savedContactId, contactStableId: contact.stableId, contactOrigin: contact.origin }; if (isDuplicateGuest(candidate, guests)) return "Este contacto ya está incluido"; setDraftGuest(candidate); setFieldErrors({}); return null; }} /></div><div className="grid min-w-0 max-w-full gap-3 sm:grid-cols-2"><div className="min-w-0 space-y-2"><Label htmlFor="guestPhone">Teléfono (opcional)</Label><Input id="guestPhone" className="h-14 w-full min-w-0 max-w-full text-base" value={draftGuest.phone ?? ""} onChange={(event) => setDraftGuest((current) => ({ ...current, phone: event.target.value }))} /></div><div className="min-w-0 space-y-2"><Label htmlFor="guestNotes">Nota (opcional)</Label><Input id="guestNotes" className="h-14 w-full min-w-0 max-w-full text-base" value={draftGuest.notes ?? ""} onChange={(event) => setDraftGuest((current) => ({ ...current, notes: event.target.value }))} /></div></div><label className="flex min-h-11 items-center gap-3 text-sm font-semibold"><input className="h-5 w-5 shrink-0 accent-[#1446cc]" checked={draftGuest.allowsCompanions} onChange={(event) => setDraftGuest((current) => ({ ...current, allowsCompanions:event.target.checked,maxCompanions:event.target.checked?Math.max(current.maxCompanions ?? 0,1):0 }))} type="checkbox" />Permitir acompañantes para esta persona</label>{draftGuest.allowsCompanions ? <CompanionLimitSelect ariaLabel="Máximo de acompañantes para esta persona" value={draftGuest.maxCompanions ?? 1} onChange={(value) => setDraftGuest((current) => ({...current,maxCompanions:value}))} /> : null}<Button className="h-14 w-full min-w-0 max-w-full" onClick={addGuest} type="button" variant="outline"><Plus className="h-5 w-5" /> Agregar a la lista</Button></div> : <button className="flex min-h-44 w-full min-w-0 max-w-full flex-col items-center justify-center rounded-2xl border border-dashed border-primary/40 bg-primary/5 p-6 text-center" onClick={() => fileInputRef.current?.click()} type="button"><Upload className="h-8 w-8 text-primary" /><span className="mt-3 font-bold">Seleccionar archivo CSV</span><span className="mt-1 break-words text-sm text-muted-foreground">Columnas: nombre, teléfono, notas, acompañantes y max_acompanantes.</span><input ref={fileInputRef} accept=".csv,text/csv" className="hidden" onChange={importCsv} type="file" /></button>}
          ...
          <div className="min-w-0 max-w-full space-y-2">{guests.map((guest, index) => <div className="flex min-w-0 max-w-full items-center gap-2 rounded-2xl bg-surface p-3 sm:gap-3" key={`${guest.fullName}-${guest.phone ?? ""}-${index}`}><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-secondary text-sm font-bold text-primary">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{guest.fullName}</span><span className="block truncate text-xs text-muted-foreground">{[guest.phone, guest.notes, guest.allowsCompanions ? `hasta ${guest.maxCompanions} acompañantes` : null].filter(Boolean).join(" · ") || "Sin datos adicionales"}</span></span><Button className="shrink-0" aria-label={`Editar a ${guest.fullName}`} onClick={() => editGuest(index)} size="icon" type="button" variant="ghost"><Pencil className="h-4 w-4" /></Button><Button className="shrink-0" aria-label={`Quitar a ${guest.fullName}`} onClick={() => setGuests((current) => current.filter((_, item) => item !== index))} size="icon" type="button" variant="ghost"><Trash2 className="h-4 w-4" /></Button></div>)}</div>
